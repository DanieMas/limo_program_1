...
import os
def generate_script(subject, video_length, creativity, reciever, api_key):
    ...
    search = WikipediaAPIWrapper(lang="zh") #维基百科返回内容语言
    search_result = search.run(subject) #维基百科获取搜索内容

    script_content = script_template.invoke({"title":title, "duration":video_length, 
                                             "people":reciever, "wikipedia_search":search_result}).content
